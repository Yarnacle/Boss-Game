...
namespace Boss_Game_2._0.Screens.Bosses
{
    class Boss1Screen : BossScreen
    {
        ...
        {
            bossAttack = new RandomCircles(spriteBatch, 0, 0, (float)Math.PI / 12, (float)Math.PI / 12, 250, 0, 0, 15, 10, 300); // Can't leave null
            rng = new Random();
            bossHP = 20;
            bossDmg = 2;
            xpDrop = 5;
        ...
        private void switchAttack()
        {
            ...
            {
                case 0:
                    bossAttack = new TopBottom(spriteBatch, 0, -0, (float)Math.PI / 15, (float)Math.PI / 2, 200, 0, 0, 15, 10, 120);
                    break;
                case 1:
                    bossAttack = new Spiral(spriteBatch, 400, 250, (float)Math.PI / 31, (float)Math.PI / 2.2f, 250, 0, 0, 15, 10, 1);
                    break;
                case 2:
                    bossAttack = new MovingCircles(spriteBatch, 0, 0, (float)Math.PI / 12, (float)Math.PI / 2, 150, 0, 0, 15, 9, false);
                    break;
                case 3:
                    bossAttack = new RandomCircles(spriteBatch, 0, 0, (float)Math.PI / 12, (float)Math.PI / 12, 150, 0, 0, 15, 10, 240);
                    break;
            }
        ...
        public override void Draw()
        {
            ...
            spriteBatch.Draw(TextureManager.Bosses.Boss1.Boss, new Rectangle(bossX - 126, bossY, 384, 384), new Rectangle(0, sourceRectY, 384, 384), Color.White); // TODO: make a boss class that will have its own .Draw() method with this code
